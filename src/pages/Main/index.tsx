...
import { useMongo, useOnline, useParams, useSaveConstituent } from "../../hooks"
...
export function Main({ sas, constituents, locations, settings }: MainI) {
    ...
    return (
        <>
            ...
            {mode === 'authenticate' && <Authenticate constituents={constituents} setConstituent={(c: constituentT) => set_Constituent(c)} phone={phone} setPhone={(p: string) => setPhone(p)} setMode={(m: string) => setMode(m)} />}

            {mode === 'signin' && <SignIn constituent={_constituent} locations={locations} activities={activities && activities[0]} />}
            {mode === 'signup' && <SignUp />}
